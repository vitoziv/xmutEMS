...
//		strBuf=String.valueOf(str.hashCode());
	...
	public static void initExternalDir(boolean cleanFile)
	{
		...
		{
			File external = new File(Constant.EXTERNAL_DIR);
			if(!external.exists())
			{
				external.mkdirs();
			}
			//check the cache whether exist
			File cache = new File(Constant.CACHE_DIR);
			if(!cache.exists())
			{
				cache.mkdirs();
			}
			...
			//check the log dir
			File logs = new File(Constant.LOGS_DIR);
			if(!logs.exists())
			{
				logs.mkdirs();
			}
			...
			File files = new File(Constant.FILES_DIR);
			if(!files.exists())
			{
				files.mkdirs();
			}
			...
			File data = new File(Constant.DATA_DIR);
			if(!data.exists())
			{
				data.mkdirs();
			}
