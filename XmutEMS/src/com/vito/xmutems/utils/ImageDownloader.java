...
public class ImageDownloader {
    ...
    private final CacheBitmap cacheBitmap = CacheBitmap.getInstance();
    private Context context;
    public enum Mode { NO_ASYNC_TASK, NO_DOWNLOADED_DRAWABLE, CORRECT }
    private Mode mode = Mode.CORRECT;

	private Bitmap watermark;//设置水印
	private int pixels = 0;//设置圆角
	private Bitmap mPlaceHolderBitmap;//设置图片加载完成前显示的图片
	private Animation anim;//设置图片出现时的动画
    ...
    public void download(String url, ImageView imageView) {
    	cacheBitmap.resetPurgeTimer();
        Bitmap bitmap = cacheBitmap.getBitmapFromCache(url);
        ...
        if (cancelPotentialDownload(url, imageView)) {
            switch (mode) {
                case NO_ASYNC_TASK:
                    ...
                    //Add other effect
                    bitmap = recreateBitmap(bitmap);
                    cacheBitmap.addBitmapToCache(url, bitmap);
                    imageView.setImageBitmap(bitmap);
                    //Add animation effect when displaying
                    if(anim!=null) {
...
//            }
    ...
    private Bitmap recreateBitmap(Bitmap bitmap) {
    	if(pixels>0){
    		bitmap = ImageUtils.toRoundCorner(bitmap, pixels);
    	}
    	if(watermark!=null) {
    		bitmap = ImageUtils.createBitmap(bitmap, watermark);
    	}
    	return bitmap;
    }
	...
	public void setmPlaceHolderBitmap(Bitmap mPlaceHolderBitmap) {
